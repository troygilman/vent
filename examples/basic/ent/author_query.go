...
func (_q *AuthorQuery) loadBooks(ctx context.Context, query *BookQuery, nodes []*Author, init func(*Author), assign func(*Author, *Book)) error {
	...
	for _, n := range neighbors {
		fk := n.book_author
		if fk == nil {
			return fmt.Errorf(`foreign-key "book_author" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "book_author" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
