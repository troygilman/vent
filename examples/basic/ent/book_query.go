...
func (_q *BookQuery) QueryReviews() *ReviewQuery {
...
func (_q *BookQuery) WithReviews(opts ...func(*ReviewQuery)) *BookQuery {
	query := (&ReviewClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BookQuery) loadAuthor(ctx context.Context, query *AuthorQuery, nodes []*Book, init func(*Book), assign func(*Book, *Author)) error {
	...
	for i := range nodes {
		if nodes[i].book_author == nil {
			continue
		}
		fk := *nodes[i].book_author
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "book_author" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *BookQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*Book, init func(*Book), assign func(*Book, *Review)) error {
