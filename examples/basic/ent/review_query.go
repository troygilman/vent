...
func (_q *ReviewQuery) QueryBook() *BookQuery {
...
func (_q *ReviewQuery) WithBook(opts ...func(*BookQuery)) *ReviewQuery {
	query := (&BookClient{config: _q.config}).Query()
...
func (_q *ReviewQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Review, init func(*Review), assign func(*Review, *Book)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].book_reviews == nil {
			continue
		}
		fk := *nodes[i].book_reviews
