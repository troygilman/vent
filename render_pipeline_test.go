...
func TestBuildProjectedRenderConfigReadOnlyFields(t *testing.T) {
	...
	permissionGroups := findSurfaceMember(t, rc.AdminSurface, "permission_groups")
	if !permissionGroups.BindUpdate {
		t.Fatal("permission_groups BindUpdate = false, want true")
	}
	for _, field := range rc.UpdateInputFields {
		if field.Name == "name" {
			t.Fatal("UpdateInputFields includes name, want permission_groups only")
		}
	}
}
