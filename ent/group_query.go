...
func (_q *GroupQuery) QueryPermissions() *PermissionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, group.PermissionsTable, group.PermissionsColumn),
		)
...
func (_q *GroupQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Group, init func(*Group), assign func(*Group, *Permission)) error {
	...
	query.Where(predicate.Permission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.PermissionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_permissions
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_permissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_permissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
