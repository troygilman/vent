...
3. Check the session. If you are already on a logged-in admin page, that is the sanity check. Leave it alone. Do not log out. Do not log out just to log back in. Only log in when the session is not authenticated (for example the login page is showing), using `admin` / `test_user`.
